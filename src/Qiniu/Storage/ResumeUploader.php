...
use QiniuOSS\Enum\SplitUploadVersion;
use QiniuOSS\Http\RequestOptions;
...
final class ResumeUploader
{
    ...
    public function __construct(
        ...
        list($accessKey, $bucket, $err) = \QiniuOSS\explodeUpToken($upToken);
        $this->bucket = $bucket;
        if ($err != null) {
    ...
    private function uploadV1($fname, $blkputRets = null)
    {
        ...
        while ($uploaded < $this->size) {
            ...
            if ($response->statusCode < 0) {
                list($accessKey, $bucket, $err) = \QiniuOSS\explodeUpToken($this->upToken);
                if ($err != null) {
                    return array(null, $err);
                }
    ...
    private function uploadV2($fname, $blkputRets = null)
    {
        $uploaded = 0;
        $partNumber = 1;
        $encodedObjectName = $this->key ? \QiniuOSS\base64_urlSafeEncode($this->key) : '~';
        $isResumeUpload = $blkputRets !== null;
    ...
    private function fileUrl($fname)
    {
        ...
        $url .= '/mimeType/' . \QiniuOSS\base64_urlSafeEncode($this->mime);
        if ($this->key != null) {
            $url .= '/key/' . \QiniuOSS\base64_urlSafeEncode($this->key);
        }
        $url .= '/fname/' . \QiniuOSS\base64_urlSafeEncode($fname);
        if (!empty($this->params)) {
            foreach ($this->params as $key => $value) {
                $val = \QiniuOSS\base64_urlSafeEncode($value);
                $url .= "/$key/$val";
            }
        }
        return $url;
    ...
    private function completeParts($fname, $uploadId, $encodedObjectName)
    {
        ...
        $sortedEtags = \QiniuOSS\arraySort($etags, 'partNumber');
        $metadata = array();
        $customVars = array();
        if ($this->params) {
