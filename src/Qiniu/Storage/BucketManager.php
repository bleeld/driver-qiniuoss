...
namespace QiniuOSS\Storage;

use QiniuOSS\Auth;
use QiniuOSS\Config;
use QiniuOSS\Http\Error;
use QiniuOSS\Http\Client;
use QiniuOSS\Http\Proxy;
use QiniuOSS\Http\Response;
...
final class BucketManager
{
    ...
    public function listFiles(
        ...
        \QiniuOSS\setWithoutEmpty($query, 'prefix', $prefix);
        \QiniuOSS\setWithoutEmpty($query, 'marker', $marker);
        \QiniuOSS\setWithoutEmpty($query, 'limit', $limit);
        \QiniuOSS\setWithoutEmpty($query, 'delimiter', $delimiter);
    ...
    public function listFilesv2(
        ...
        \QiniuOSS\setWithoutEmpty($query, 'limit', $limit);
        \QiniuOSS\setWithoutEmpty($query, 'delimiter', $delimiter);
        \QiniuOSS\setWithoutEmpty($query, 'skipconfirm', $skipconfirm);
    ...
    {
        $path = '/stat/' . \QiniuOSS\entry($bucket, $key);
    ...
    {
        $path = '/delete/' . \QiniuOSS\entry($bucket, $key);
    ...
    {
        $from = \QiniuOSS\entry($from_bucket, $from_key);
        $to = \QiniuOSS\entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        if ($force === true) {
            $path .= '/force/true';
        }
    ...
    {
        $resource = \QiniuOSS\entry($bucket, $key);
        $encode_mime = \QiniuOSS\base64_urlSafeEncode($mime);
    ...
    public function fetch($url, $bucket, $key = null)
    {
        ...
        $resource = \QiniuOSS\base64_urlSafeEncode($url);
        $to = \QiniuOSS\entry($bucket, $key);
    ...
    public function asynchFetch(
        ...
        \QiniuOSS\setWithoutEmpty($params, 'host', $host);
        \QiniuOSS\setWithoutEmpty($params, 'key', $key);
        \QiniuOSS\setWithoutEmpty($params, 'md5', $md5);
        \QiniuOSS\setWithoutEmpty($params, 'etag', $etag);
        \QiniuOSS\setWithoutEmpty($params, 'callbackurl', $callbackurl);
        \QiniuOSS\setWithoutEmpty($params, 'callbackbody', $callbackbody);
        \QiniuOSS\setWithoutEmpty($params, 'callbackbodytype', $callbackbodytype);
        \QiniuOSS\setWithoutEmpty($params, 'callbackhost', $callbackhost);
        \QiniuOSS\setWithoutEmpty($params, 'file_type', $file_type);
        \QiniuOSS\setWithoutEmpty($params, 'ignore_same_key', $ignore_same_key);
    ...
    public function batch($operations)
    {
        ...
        foreach ($operations as $op) {
            ...
            list($bucket,) = \QiniuOSS\decodeEntry($segments[2]);
        }
    ...
    {
        $entry = \QiniuOSS\entry($bucket, $key);
    ...
    public function setObjectLifecycleWithCond(
        ...
        $encodedEntry = \QiniuOSS\entry($bucket, $key);
        ...
        if ($cond != null) {
            ...
            $path .= '/cond' . \QiniuOSS\base64_urlSafeEncode($condStr);
        }
        return $this->rsPost($bucket, $path);
    }
    ...
    public static function buildBatchDeleteAfterDays($bucket, $key_day_pairs)
    {
        ...
        foreach ($key_day_pairs as $key => $day) {
            array_push($data, '/deleteAfterDays/' . \QiniuOSS\entry($bucket, $key) . '/' . $day);
        }
        return $data;
    }
    ...
    public static function buildBatchChangeMime($bucket, $key_mime_pairs)
    {
        ...
        foreach ($key_mime_pairs as $key => $mime) {
            array_push($data, '/chgm/' . \QiniuOSS\entry($bucket, $key) . '/mime/' . base64_encode($mime));
        }
        return $data;
    }
    ...
    public static function buildBatchChangeType($bucket, $key_type_pairs)
    {
        ...
        foreach ($key_type_pairs as $key => $type) {
            array_push($data, '/chtype/' . \QiniuOSS\entry($bucket, $key) . '/type/' . $type);
        }
        return $data;
    }
    ...
    public static function buildBatchRestoreAr($bucket, $key_restore_days_pairs)
    {
        ...
        foreach ($key_restore_days_pairs as $key => $restore_days) {
            array_push($data, '/restoreAr/' . \QiniuOSS\entry($bucket, $key) . '/freezeAfterDays/' . $restore_days);
        }
        return $data;
    }
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . \QiniuOSS\entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket, $force)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = \QiniuOSS\entry($source_bucket, $from_key);
            $to = \QiniuOSS\entry($target_bucket, $to_key);
